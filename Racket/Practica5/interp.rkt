...
(define (interp expr env)
 (match expr
   ...
   [(num n) (numV n)]
   [(bool b) (boolV b)]
   [(op f l) (let ([op-val (evalua-op f l env)])
               (if (boolean? op-val) (boolV op-val) (numV op-val)))]
   [(fun param body) (closureV param body env)]
   [(iF test-expr then-expr else-expr)
    (if (interp-op test-expr env) (interp then-expr env) (interp else-expr env))]
   [(app fun-exp arg) (let ([fun-val (interp fun-exp env)])
                        (interp (get-e fun-val "body")
                                (aSub (get-e fun-val "param")
                                      (interp arg env) (get-e fun-val "env"))))]))
...
;; Análisis semántico para op e iF
;; Hace las interpretaciones de los valores puros sin boolV o numV
;; interp: AST Env → AST-Value
